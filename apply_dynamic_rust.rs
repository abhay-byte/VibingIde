...
fn main() {
    ...
    let start_idx = content.find("fn render_side_nav_bar(&mut self, ctx: &egui::Context) {");
    let end_idx = content.find("    fn render_mobile_bottom_nav(&mut self, ctx: &egui::Context) {");

    if start_idx.is_none() || end_idx.is_none() {
        println!("Could not find targets!");
        return;
    }
    
    let new_code = r#"fn render_side_nav_bar(&mut self, ctx: &egui::Context) {
        egui::SidePanel::left("ultra_slim_rail")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ...
                    ui.add_space(24.0);
                    if ui.add(egui::Button::new(RichText::new(egui_phosphor::regular::ROBOT_LION).size(22.0).color(if self.current_screen == AppScreen::Agents { ACCENT } else { TEXT_DIM })).frame(false)).clicked() {
                    ...
                    if self.current_screen == AppScreen::Settings {
                        ...
                        let g_rect = Default::default(); // we can't easily draw custom rect without taking space, let's just color it
            ...
            .show(ctx, |ui| {
                ui.horizontal_centered(|ui| {
                    ...
                    if self.current_screen == AppScreen::Agents {
                        ...
                        ui.label(RichText::new("VibingIDE").color(ACCENT).size(16.0).strong());
                        ui.add_space(32.0);
...
"#;
    ...
    let pre = &content[..start_idx.unwrap()];
    let post = &content[end_idx.unwrap()..];
    let new_content = format!("{}{}\n{}", pre, new_code, post);

    fs::write("src/app.rs", new_content).unwrap();
